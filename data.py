...
class Data(object):
    ...
    @staticmethod
    def load_dataframe(filename, load_semantic_maps=False):  #called in train() function
        print('Inside load dataframe')
        df = pd.read_hdf(filename, key='df').sample(frac=1).reset_index(drop=True) 
        '''
        pd.read_hdf() creates dataframe by reading hdf files at path 'filename', and returns it.
        [dataframe].sample(frac=1) randomly selects all the dataframe entries (since frac=1. if frac=0.1, 10% of entries are randomly selected).
        [dataframe].reset_index(drop=True) resets the indices of the entries in [dataframe], in order. for eg:

        698   data/leftImg8bit/train/darmstadt/darmstadt_000...
        153   data/leftImg8bit/train/stuttgart/stuttgart_000...
        850   data/leftImg8bit/train/bremen/bremen_000113_00...
        590   data/leftImg8bit/train/dusseldorf/dusseldorf_0...

        becomes :-

        0   data/leftImg8bit/train/darmstadt/darmstadt_000...
        1   data/leftImg8bit/train/stuttgart/stuttgart_000...
        2   data/leftImg8bit/train/bremen/bremen_000113_00...
        3   data/leftImg8bit/train/dusseldorf/dusseldorf_0...

        https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.sample.html - check for argument list.
        '''        
        if load_semantic_maps: 
            return df['path'].values, df['semantic_map_path'].values
            
            #this if statement will run only when conditional_GAN is true
            
        else:
            return df['path'].values
            '''
            returns only 'path' entries from dataframe and no other column. the return type is a numpy array.
            https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.values.html - for reference
            '''
